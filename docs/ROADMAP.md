...
| 0 — Contrat SSDV2 | `ssdv2ctl` non interactif JSON (apps, app, auth, diagnostics) | À faire |
| 1 — Socle WebUI | FastAPI + React/Vite + shadcn + SQLite + auth admin + image Docker + healthcheck | À faire |
| 2 — Lecture seule | Dashboard, catalogue ~190 apps, filtres, détail app, conteneurs, logs | À faire |
